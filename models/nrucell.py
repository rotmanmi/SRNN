...
class NRUWrapper(nn.Module):
    def __init__(self, input_size, output_size, hidden_size, *args, **kwargs):
        super().__init__()
        self.single_output = kwargs['single_output']
        if kwargs['embedding']:
            self.embedding = nn.Embedding(input_size, hidden_size)
            self.rnncell = NRU('cuda', hidden_size, output_size, num_layers=1, layer_size=[hidden_size],
                               output_activation="linear", layer_norm=False, use_relu=False, memory_size=256, k=4)
        else:
            self.rnncell = NRU('cuda', input_size, output_size, num_layers=1, layer_size=[hidden_size],
                               output_activation="linear", layer_norm=False, use_relu=False, memory_size=256, k=4)
